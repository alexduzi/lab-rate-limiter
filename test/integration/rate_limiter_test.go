...
func setupRedis(ctx context.Context, t *testing.T) (*tcredis.RedisContainer, string) {
	t.Helper()
	...
	return redisContainer, parsed.Host
}
...
func setupServer(t *testing.T) (*httptest.Server, *http.Client) {
	...
	redisContainer, connectionString := setupRedis(ctx, t)

	store, err := storage.NewRedisStorage(connectionString, "", 0)
	if err != nil {
	...
	t.Cleanup(func() {
		...
		if err := store.Close(); err != nil {
			t.Errorf("failed to close redis storage: %v", err)
		}
